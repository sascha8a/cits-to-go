...
This firmware is intended for the **Seeed Studio XIAO ESP32-C5**. It captures C-ITS packets, pulses the onboard USER LED when packets are accepted, and streams the captured packets to an Android phone over USB serial and/or to another device over GPIO UART.
...
- Streams accepted packets as compact COBS-delimited binary records over USB Serial/JTAG, GPIO UART, or both.
...
The firmware writes one COBS record per captured packet to the configured serial output. Each encoded record is terminated by a single `0x00` delimiter.
...
- `CONFIG_CITS_SERIAL_OUTPUT_UART`: write packet records to GPIO UART only.
- `CONFIG_CITS_SERIAL_OUTPUT_USB_AND_UART`: write packet records to both USB Serial/JTAG and GPIO UART.
- `CONFIG_CITS_UART_BAUD_RATE`: GPIO UART baud rate, default `921600`.
- `CONFIG_CITS_UART_TX_GPIO`: GPIO UART TX pin, default GPIO0, which is XIAO pin D1.
- `CONFIG_CITS_UART_RX_GPIO`: GPIO UART RX pin, default GPIO25, which is XIAO pin D2. Set to `-1` if only TX is needed.
- `CONFIG_CITS_LED_GPIO`: USER LED GPIO. The XIAO ESP32-C5 USER LED is GPIO27 and active-low.

For a second XIAO ESP32-C5 receiver board, connect GND to GND and cross the UART lines: this firmware's TX pin goes to the other board's RX pin, and this firmware's RX pin goes to the other board's TX pin if bidirectional serial is needed. The default GPIO UART configuration uses D1/GPIO0 for TX and D2/GPIO25 for RX; D3 through D5 remain available unless you assign them in `idf.py menuconfig`.
