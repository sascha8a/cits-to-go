...
This firmware is intended for the **Seeed Studio XIAO ESP32-C5**. It captures C-ITS packets, pulses the onboard USER LED when packets are accepted, and streams the captured packets to an Android phone over USB serial and/or to another device over GPIO SPI.
...
- Streams accepted packets as compact COBS-delimited binary records over USB Serial/JTAG, GPIO SPI, or both.
...
## Packet transport format

The firmware writes one COBS record per captured packet to the configured output. Each encoded record is terminated by a single `0x00` delimiter. USB Serial/JTAG and GPIO SPI carry the same CTG1 byte stream.
...
COBS framing means raw `0x00` bytes never appear inside an encoded record, so readers can resynchronize by scanning for the delimiter.
...
- `CONFIG_CITS_SERIAL_OUTPUT_USB`: write packet records to USB Serial/JTAG only.
- `CONFIG_CITS_SERIAL_OUTPUT_SPI`: write packet records to GPIO SPI only.
- `CONFIG_CITS_SERIAL_OUTPUT_USB_AND_SPI`: write packet records to both USB Serial/JTAG and GPIO SPI.
- `CONFIG_CITS_SPI_CLOCK_HZ`: GPIO SPI clock, default `4000000`.
- `CONFIG_CITS_SPI_CS_GPIO`: SPI CS pin, default GPIO0, which is XIAO pin D1.
- `CONFIG_CITS_SPI_READY_GPIO`: SPI ready pin, default GPIO25, which is XIAO pin D2.
- `CONFIG_CITS_SPI_MOSI_GPIO`: SPI MOSI pin, default GPIO7, which is XIAO pin D3.
- `CONFIG_CITS_SPI_MISO_GPIO`: SPI MISO pin, default GPIO23, which is XIAO pin D4.
- `CONFIG_CITS_SPI_CLK_GPIO`: SPI clock pin, default GPIO24, which is XIAO pin D5.
- `CONFIG_CITS_LED_GPIO`: USER LED GPIO. The XIAO ESP32-C5 USER LED is GPIO27 and active-low.

For a second XIAO ESP32-C5 uplink board, connect GND to GND and connect D1 through D5 straight through. The capture firmware is the SPI master; the Wi-Fi uplink firmware is the SPI slave.
