...
The ESP32-C5 firmware listens for C-ITS packets, forwards them over USB serial and optionally over GPIO UART, and the Android app can save the captured USB data as PCAP and forward the packets to an MQTT broker using the same packet-topic style as the original OpenTrafficMap receiver firmware.
...
- Optionally mirror captured packets to another device over GPIO UART.
...
- Optional: second microcontroller or serial receiver connected to the XIAO GPIO UART pins
...
- Optionally send the same packet frames over GPIO UART, defaulting to XIAO D1/GPIO0 TX and D2/GPIO25 RX at 921600 baud.

### ESP32-C5 Wi-Fi uplink firmware

The optional `wifi-uplink-firmware/` project runs on a second XIAO ESP32-C5. It receives the capture board's CTG1 serial frames over GPIO UART and publishes the raw packet payloads directly to MQTT over Wi-Fi.
